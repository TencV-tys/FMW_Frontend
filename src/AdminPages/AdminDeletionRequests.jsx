...
export default function AdminDeletionRequests() {
  ...
  const handleProcessDeletionRequest = async (requestId, action, adminNotes = '') => {
    try {
      ...
      if (response.ok) {
        ...
        // Update local state - remove the processed request
        setDeletionRequests(prevRequests => 
          prevRequests.filter(request => request.id !== requestId)
        );
        
        // Refresh user stats since deletion counts may have changed
        fetchUsersDeletionStats();
  ...
  const stats = {
    ...
    limitReached: filteredUsers.filter(u => u.limit_reached).length,
    approachingLimit: filteredUsers.filter(u => u.deletion_count >= 2 && !u.limit_reached).length,
    totalDeletions: filteredUsers.reduce((sum, user) => sum + (user.deletion_count || 0), 0),
  ...
  return (
    <div className="admin-deletion-requests-page">
      <AdminNav isOpen={isNavOpen} setIsOpen={setIsNavOpen} />
      
      <main className={`admin-deletion-content ${isNavOpen ? 'nav-open' : 'nav-closed'}`}>
        {/* Header */}
        ...
        <div className="users-stats">
          <div className="stat-card">
            <span className="user-stat-number">{stats.pendingRequests}</span>
